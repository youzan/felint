...
program.version(versionUtil.VERSION).command('init').option('-p, --plan [value]', '使用指定代码规范方案').description('使用felint初始化项目。更多信息请参考：https://github.com/youzan/felint/blob/master/README.md').action((() => {
    var _ref = _asyncToGenerator(function* (options) {
        ...
        if (!isUpdating) {
            ...
            ruleFile.createPlan(plan).then(function () {
...
program.command('dep').description('安装 eslint/stylelint 及其依赖，并写入package.json').action(_asyncToGenerator(function* () {
    ...
    if (!isUpdating) {
        ...
        let msgInfo = yield dependence.install(plan).then(function () {
            sh.exec('rm -rf ./.felint', { silent: true });
        });
        console.log(msgInfo.join('\n'));
    }
...
// 更新规则文件
program.command('rules').description('更新规则文件').action(_asyncToGenerator(function* () {
    const felintrcFile = felintrc.read();
    // 拉取最新配置
    yield fetchConfig(felintrcFile || {});
