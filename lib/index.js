...
// 更新配置文件和钩子
program.command('update').description('更新felint的配置文件').action(_asyncToGenerator(function* () {
    let isUpdating = yield versionUtil.checkUpdate();
    if (isUpdating) {
        return;
    }
    ...
    if (!isLocal) {
        ...
        yield dependence.install(!isLocal);
        updateHooks.update();
    } else {
...
program.command('export').option('-p, --plan [value]', '使用指定代码规范方案').description('创建本地对应依赖').action((() => {
...
// 判断该项目是否是local项目
program.command('islocal').description('查看该项目是否是local项目').action(() => {
